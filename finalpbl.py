...
Original file is located at
    https://colab.research.google.com/drive/1TZwvoM-_wZvi5ZfItUTbEemd-J9nSfe3
"""
...
sheet_name = "WSN_Sensor_Data"
url = f"https://docs.google.com/spreadsheets/d/{sheet_id}/gviz/tq?tqx=out:csv&sheet={sheet_name}"
df = pd.read_csv(url)

# STEP 2: Confirm needed columns
required_cols = ['MAC address', 'Value']
for col in required_cols:
    if col not in df.columns:
        raise ValueError(f"Missing required column: {col}")

# STEP 3: Clean data types
df['MAC address'] = df['MAC address'].astype('category')  # ✅ Fix: treat MAC as category
df['Value'] = pd.to_numeric(df['Value'], errors='coerce')
df.dropna(subset=['MAC address', 'Value'], inplace=True)
...
# STEP 4: Violin Plot
plt.figure(figsize=(12, 1.2 * df['MAC address'].nunique()))
sns.violinplot(data=df, x='Value', y='MAC address', inner='stick', palette='Dark2')
sns.despine()
plt.title("Sensor Value Distribution by MAC Address")
plt.xlabel("Sensor Reading (Value)")
plt.ylabel("MAC Address (as Category)")
plt.tight_layout()
plt.show()
...
df['35'].plot(kind='hist', bins=20, title='35')
plt.gca().spines[['top', 'right',]].set_visible(False)

# @title 35

from matplotlib import pyplot as plt
df['35'].plot(kind='line', figsize=(8, 4), title='35')
plt.gca().spines[['top', 'right']].set_visible(False)
...
df_sheet_processed = preprocess_data(df_sheet)
X_sheet, _, _, _ = prepare_features_targets(df_sheet_processed)

# --- 3. Load Trained Models ---
import joblib

# Adjust path if models are stored in a custom directory
temp_model = joblib.load("models/temperature_prediction_model.pkl")
humidity_model = joblib.load("models/humidity_prediction_model.pkl")

# --- 4. Predict Temperature and Humidity ---
predicted_results = predict_future_weather(X_sheet, temp_model, humidity_model)

# --- 5. Display Predictions with Confidence Ranges ---
print("Predicted Temperature and Humidity from Google Sheet Data:\n")
print(predicted_results.head())

# Optional: save to CSV or export back to Google Sheets
predicted_results.to_csv("google_sheet_predictions.csv", index=False)